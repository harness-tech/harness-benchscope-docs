...
::: info
阈值判定所使用的统计量可通过参数调整（如 `--ttft-statistic p99`），以适配不同的业务口径。详见 [CLI 参考](../get-started/cli.md)。
:::
...
- [CLI 参考](../get-started/cli.md) — `perf` 命令完整参数
- [教程：并发压测](../tutorials/perf-concurrency.md) — 分步操作
- [教程：阈值压测](../tutorials/perf-threshold.md) — 求最优并发
- [设置（Settings）](./settings.md) — 配置 Provider 与 Bench Engines
- [数据与统计（Datas）](./datas.md) — 历史记录与导入
