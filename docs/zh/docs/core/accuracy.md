...
::: tip
精度测试专注于「模型输出是否正确」，与[性能测试](./performance.md)互补：性能回答「跑得多快」，精度回答「答得多准」。
:::
...
数据集可传**内置数据集 id**（`mmlu` / `gsm8k` 等）或**本地 JSONL 路径**，见 [设置（Settings）→ Datasets](./settings.md)。
...
- [CLI 参考](../get-started/cli.md) — `eval` 命令完整参数
- [教程：精度评测](../tutorials/accuracy-guide.md) — 分步操作
- [数据与统计（Datas）](./datas.md) — 结果查看与导入
- [设置（Settings）](./settings.md) — 配置 Provider、Datasets
