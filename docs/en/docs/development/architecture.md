...
Configuration is persisted by `config.py` to `~/.benchscope/settings.json`. The **data root** can be overridden via `BENCHSCOPE_DATA_DIR`. All task artifacts and cache directories live under the data root — see [Configuration](../get-started/configuration.md) for the full layout.
...
It includes **environment validation** and **parameter descriptions**, with **explicit metric availability** for third-party engines. See [Bench Engine](bench-engine.md) for details.
...
- [Bench Engine](bench-engine.md) — the engine abstraction
- [Contributing](contributing.md) — how to develop against this architecture
