...
在**已知业务 SLA** 时，用阈值探测自动找到满足条件点的**最大并发**（`best_concurrency`），回答「这套配置最多能扛住多少并发且不超 SLA」。

## 设定业务指标

例如业务要求：

- **TTFT ≤ 200ms**（首 token 延迟）；
- **TPOT ≤ 100ms**（每输出 token 延迟）。

用阈值探测即可自动搜索同时满足这两项条件的最大并发。

::: tip
阈值条件可加**输出吞吐下限**（`--output-threshold`），并要求吞吐不低于某值才算达标，详见 [CLI 参考](../get-started/cli.md)。
:::
...
- 可调整判定统计量（`--ttft-statistic p99` 等）以适配**不同的业务口径**。
...
**问题：搜索上限设多大合适？**
从业务可接受的规模估计，如未知可先设 512–1024；越大探测时间越长。
...
- [性能测试](../core/performance.md) — 阈值模式原理与判定条件
- [CLI 参考](../get-started/cli.md) — 阈值模式专属参数
- [教程：并发压测](./perf-concurrency.md) — 手动逐档压测
